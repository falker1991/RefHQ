...
test("Version 0.35.2 uses the dashboard loading label, favicon metadata, and preserved Worker secrets", async () => {
  ...
  assert.equal(JSON.parse(packageJson).version, "0.35.2");
...
test("v0.35.2 shows the application version beside Help for the Site Owner only", async () => {
  const [page, css, packageJson] = await Promise.all([read("app/page.tsx"), read("app/globals.css"), read("package.json")]);
  assert.match(page, /const APP_VERSION = "0\.35\.2"/);
...
test("v0.35.2 gives combined Site Supervisor and Referee Coach users the Ratings tab", async () => {
...
test("v0.35.2 creates paper-efficient schedule PDFs with one dynamic column per crew position", async () => {
...
test("v0.31.2 provides prominent schedule access on every dashboard", async () => {
  ...
  assert.match(page, /className="dashboard-schedule-link" onClick={\(\) => onNavigate\(profile\.role === "referee" \? "board" : "schedule"\)}/);
  assert.match(page, /View My Assignments/);
  assert.match(page, /View Event Schedule/);
...
test("v0.35.2 omits Skills Eval-only columns from Basic Eval-only exports", async () => {
  const [page, packageJson] = await Promise.all([read("app/page.tsx"), read("package.json")]);
...
test("v0.35.2 keeps duplicate game-rating submissions adjacent in deterministic export order", async () => {
  const [page, packageJson] = await Promise.all([read("app/page.tsx"), read("package.json")]);
...
test("v0.35.2 names rating exports from compact active filters instead of the UTC export date", async () => {
  const [page, packageJson] = await Promise.all([read("app/page.tsx"), read("package.json")]);
...
test("v0.35.2 confirms crew-rating writes and refreshes saved data before closing", async () => {
  ...
  assert.match(page, /const savedRatings = await Promise\.all\(gameAssignments\.map/);
...
test("v0.35.2 freezes assignment-grid field and time headers on both scroll axes", async () => {
  const [page, css, packageJson] = await Promise.all([read("app/page.tsx"), read("app/globals.css"), read("package.json")]);
  ...
  assert.match(css, /\.board-wrap\{position:relative;max-height:min\(72dvh,760px\);overflow:auto;overscroll-behavior:contain\}/);
...
test("v0.35.2 automatically activates new deployments for installed and reopened apps", async () => {
