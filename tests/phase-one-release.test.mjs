...
test("Version 0.35.4 uses the dashboard loading label, favicon metadata, and preserved Worker secrets", async () => {
  ...
  assert.equal(JSON.parse(packageJson).version, "0.40.5");
...
test("coach schedule lists crews and opens the selected game's rating modal", async () => {
  ...
  assert.match(page, /onRateCrew=\{\(gameId\) => \{ setRatingEditAssessmentId\(null\); setRatingModalGameId\(gameId\); \}\}/);
...
test("v0.35.4 shows the application version beside Help for the Site Owner only", async () => {
  ...
  assert.match(page, /const APP_VERSION = "0\.40\.5"/);
...
test("v0.40.5 confirms atomic crew-rating writes before closing", async () => {
...
test("v0.36.0 improves operational filters, directory ratings, modals, and mobile navigation", async () => {
  ...
  assert.match(page, /label="Assignment Changes"/);
  assert.match(page, /changeFilters\.includes\(item\.schedule_changed_at \? "changed" : "unchanged"\)/);
...
test("v0.40.5 reduces event and crew-rating database round trips", async () => {
  const [page, client, migration, packageJson] = await Promise.all([
...
test("v0.40.5 consolidates staffing views into Schedule and repairs the mobile tray", async () => {
  ...
  assert.match(page, /Time and Field Grid/);
  assert.match(page, /embedded initialView=\{scheduleView\}/);
...
test("v0.40.5 combines rating status and authorized report access and safely reopens saved submissions", async () => {
  const [page, css, packageJson] = await Promise.all([read("app/page.tsx"), read("app/globals.css"), read("package.json")]);
...
test("v0.40.5 stacks submitted report cards at equal width in crew order", async () => {
  const [page, css, packageJson] = await Promise.all([read("app/page.tsx"), read("app/globals.css"), read("package.json")]);
  ...
  assert.match(css, /\.submitted-reports-list\{display:grid;grid-auto-flow:row;grid-template-columns:minmax\(0,1fr\)/);
  assert.match(css, /\.submitted-reports-list article\{display:grid;box-sizing:border-box;width:100%;min-width:0/);
  assert.equal(JSON.parse(packageJson).version, "0.40.5");
});

test("v0.40.5 shows only the referee's own shared evaluation in My Assignments", async () => {
  const [page, client, migration, packageJson] = await Promise.all([
    read("app/page.tsx"), read("app/supabase-client.ts"),
