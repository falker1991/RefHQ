...
test("Version 0.34.5 uses the dashboard loading label, favicon metadata, and preserved Worker secrets", async () => {
  ...
  assert.equal(JSON.parse(packageJson).version, "0.34.5");
...
test("v0.34.5 shows the application version beside Help for the Site Owner only", async () => {
  const [page, css, packageJson] = await Promise.all([read("app/page.tsx"), read("app/globals.css"), read("package.json")]);
  assert.match(page, /const APP_VERSION = "0\.34\.5"/);
...
test("v0.34.5 gives combined Site Supervisor and Referee Coach users the Ratings tab", async () => {
...
test("v0.34.5 omits Skills Eval-only columns from Basic Eval-only exports", async () => {
  const [page, packageJson] = await Promise.all([read("app/page.tsx"), read("package.json")]);
...
test("v0.34.5 keeps duplicate game-rating submissions adjacent in deterministic export order", async () => {
  const [page, packageJson] = await Promise.all([read("app/page.tsx"), read("package.json")]);
...
test("v0.34.5 names rating exports from compact active filters instead of the UTC export date", async () => {
  const [page, packageJson] = await Promise.all([read("app/page.tsx"), read("package.json")]);
...
test("v0.34.5 automatically activates new deployments for installed and reopened apps", async () => {
