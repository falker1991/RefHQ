...
test("Version 0.35.4 uses the dashboard loading label, favicon metadata, and preserved Worker secrets", async () => {
  ...
  assert.equal(JSON.parse(packageJson).version, "0.40.6");
...
test("v0.35.4 shows the application version beside Help for the Site Owner only", async () => {
  ...
  assert.match(page, /const APP_VERSION = "0\.40\.6"/);
...
test("v0.40.6 confirms atomic crew-rating writes before closing", async () => {
...
test("v0.40.6 reduces event and crew-rating database round trips", async () => {
  const [page, client, migration, packageJson] = await Promise.all([
...
test("v0.40.6 consolidates staffing views into Schedule and repairs the mobile tray", async () => {
...
test("v0.40.6 combines rating status and authorized report access and safely reopens saved submissions", async () => {
  const [page, css, packageJson] = await Promise.all([read("app/page.tsx"), read("app/globals.css"), read("package.json")]);
...
test("v0.40.6 stacks submitted report cards at equal width in crew order", async () => {
  const [page, css, packageJson] = await Promise.all([read("app/page.tsx"), read("app/globals.css"), read("package.json")]);
...
test("v0.40.6 shows only the referee's own shared evaluation in My Assignments", async () => {
  const [page, client, migration, packageJson] = await Promise.all([
    read("app/page.tsx"), read("app/supabase-client.ts"),
